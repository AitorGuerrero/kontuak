...
class MovementsHistoryTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        $entry1 = new Entry(30, 'A', new \DateTime('2015-08-01'));
        $entry2 = new Entry(100, 'B', new \DateTime('2015-08-05'));
        $expenditure1 = new Expenditure(50, 'C', new \DateTime('2015-08-04'));
    ...
    public function shouldReturnMovementsMainData()
    {
        ...
        $concept = 'A';
        $date = '2015-08-01';
        $entry1 = new Entry($amount, $concept, new \DateTime($date));
        $this->collection->add($entry1);
        $response = $this->useCase->execute($this->request);
    ...
    {
        $this->collection->add(new Entry(30, 'A', new \DateTime('2015-08-01')));
        $this->collection->add(new Entry(100, 'B', new \DateTime('2015-08-05')));
        $this->collection->add(new Expenditure(50, 'C', new \DateTime('2015-08-04')));
        $response = $this->useCase->execute($this->request);
     ...
     * @test
     */
    public function whenThereAreMoraMovementsThanLimitShouldGetCorrectTotalAmount()
    {
        $this->collection->add(new Entry(30, 'A', new \DateTime('2015-08-01')));
        $this->collection->add(new Entry(100, 'B', new \DateTime('2015-08-05')));
        $this->collection->add(new Expenditure(50, 'C', new \DateTime('2015-08-04')));
     ...
     * @test
     */
    public function shouldReturnRequestedMovementsAmount()
    {
        $requestedAmount = 2;
        $movement1 = new Expenditure(50, 'C', new \DateTime('2015-08-04'));
        $this->collection->add(new Entry(30, 'A', new \DateTime('2015-08-01')));
        $this->collection->add(new Entry(100, 'B', new \DateTime('2015-08-05')));
        $this->collection->add($movement1);
        $this->request->limit = $requestedAmount;
        $response = $this->useCase->execute($this->request);

        $this->assertEquals($requestedAmount, count($response->movements));
        $this->assertEquals($response->movements[0]['id'], $movement1->id()->serialize());
    }
